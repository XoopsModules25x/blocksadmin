...
foreach ($group_defs as $def) {
    if (mb_strstr($def, '_AM_ACCESSRIGHTS') || mb_strstr($def, constant('CO_' . $moduleDirNameUpper . '_' . 'ACTIVERIGHTS'))) {
        eval($def);
    }
}
...
function list_groups2()
{
    ...
    if ($target_mid > 1) {
        $form->addAppendix('module_admin', $target_mid, $target_mname . ' ' . constant('CO_' . $moduleDirNameUpper . '_' . 'ACCESSRIGHTS'));
